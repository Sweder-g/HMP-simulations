...
base_path = "c:\\Users\\Sweder\\Desktop\\Generated data\\"
...
for folder in folders:
    ...
    try:
        ...
        print(f"Skipping folder {folder} as it does not contain a valid amplitude.")
        continue

    # Get fif and npy files
    fif_files, npy_files = general.get_files(path)
    print(f"fif_files: {fif_files}")
    print(f"npy_files: {npy_files}")

    if not fif_files or not npy_files:
        print(f"Skipping folder {folder} due to missing .fif or .npy files.")
        continue

    signalless_fif, signalless_events = general.get_files("c:\\Users\\Sweder\\Desktop\\signalless_fif_and_events\\")
    print(f"signalless_fif: {signalless_fif}")
    print(f"signalless_events: {signalless_events}")

    # Ensure signalless files are present
    if not signalless_fif or not signalless_events:
        print("Skipping due to missing signalless files.")
        continue
    ...
    for strategy in strategies:
        ...
        if predicted_events is None:
            print(f"No events detected for strategy: {method} with {varcov_or_erp}")
            continue
        ...
        fig.suptitle(folder + " A")
        fig.subplots_adjust(hspace=0.4)
        fig.patch.set_facecolor('#F7F7F7')  # Set the figure background color
        ...
        # Set background color to slightly grey for each subplot
    ...
    # Add folder results to all_results
    all_results.extend(results)

# Save all results to a single CSV
all_results_df = pd.DataFrame(all_results)
results_csv_path = os.path.join(base_path, "all_results.csv")
