...
def simulate(sources, n_trials, n_jobs, file, data_type='eeg', n_subj=1, path='./', overwrite=False, verbose=False, noise=True, times=None, seed=None, sfreq=100, save_snr=True, save_noiseless=False, a='center', b=0.0, x=0, y=0, z=0):
    '''
    ...
    if x != 0 or y != 0 or z != 0:
        info = change_montage(info, x=x, y=y, z=z)
    ...
    for subj in range(n_subj):
        ...
        if subj_file in os.listdir(path) and not overwrite:
            ...
            label = mne.label.select_sources(subject, selected_label, subjects_dir=subjects_dir, random_state=random_state, location=a, extent=b)
...
def change_montage(info, x = 0, y = 0, z = 0):
    montage = info.get_montage()
    all_meas =  montage.get_positions()
    pos = all_meas['ch_pos']
    for elec in pos:
        pos[elec][0] += x
        pos[elec][1] += y
        pos[elec][2] += z
    all_meas['ch_pos'] = pos
    new_montage = mne.channels.make_dig_montage(all_meas['ch_pos'], nasion=all_meas['nasion'], 
            lpa = all_meas['lpa'], rpa = all_meas['rpa'], hsp = all_meas['hsp'], hpi=all_meas['hpi'])
    return info.set_montage(new_montage)